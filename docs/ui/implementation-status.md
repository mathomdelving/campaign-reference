...
- **`CRLineChart.tsx`** ✅ - Reusable chart component matching roadmap spec
  - Accepts `data`, `series`, `height`, `showLegend`, `yAxisFormatter`, `quarterlyTicks` props
  - Uses numeric X-axis with Unix timestamps for accurate chronological positioning
  - Evenly spaced quarterly ticks regardless of filing count
  - Plots all filing types (Q1-Q4, PRE-PRIMARY, PRE-GENERAL, etc.) as separate datapoints
